...
class TestPiperIntegration:
    ...
    def test_piper_transformation_pipeline(self, leader_node, piper_master_node, follower_node):
        ...
        # Verify Piper transformations were applied
        assert abs(received_state.joint_0 - 1.0) < 0.01, "Joint 0: no transformation"
        assert abs(received_state.joint_1 - 0.8) < 0.01, "Joint 1: scaled by 0.8"
        assert abs(received_state.joint_2 - 1.2) < 0.01, "Joint 2: scaled by 1.2"
        assert abs(received_state.joint_3 - (-1.0)) < 0.01, "Joint 3: inverted"
        assert abs(received_state.joint_4 - 1.0) < 0.01, "Joint 4: no transformation"
        assert abs(received_state.joint_5 - 1.0) < 0.01, "Joint 5: no transformation"
    ...
    def test_piper_joint_limits(self, leader_node, piper_master_node, follower_node):
        ...
        valid_state = JointState(
            joint_0=0.5,    # OK
            joint_1=1.5,    # OK (within ±1.8326)
            joint_2=1.8,    # OK (within ±2.0944)
            joint_3=1.0,    # OK
            joint_4=1.2,    # OK (within ±1.5708)
            joint_5=2.0,    # OK
            gripper=0.4
        )
        ...
        # Check transformations were applied to valid state
        assert abs(received_state.joint_1 - (1.5 * 0.8)) < 0.01, "Valid joint 1 transformed"
        assert abs(received_state.joint_2 - (1.8 * 1.2)) < 0.01, "Valid joint 2 transformed"
    ...
    def test_piper_joint_mapping(self, leader_node, piper_master_node, follower_node):
        ...
        # With identity mapping + transformations:
        # joint_0: 0.1 * 1.0 = 0.1
        # joint_1: 0.2 * 0.8 = 0.16  
        # joint_2: 0.3 * 1.2 = 0.36
        # joint_3: 0.4 * -1.0 = -0.4
        # joint_4: 0.5 * 1.0 = 0.5
        # joint_5: 0.6 * 1.0 = 0.6
        
        expected = [0.1, 0.16, 0.36, -0.4, 0.5, 0.6]
